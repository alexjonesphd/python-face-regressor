...
def explore(model):
    ...
    def make_document(doc):
        ...
        # Instantiate plot object for warped image
        warp_image_plot = figure(title = 'Predicted Face', y_range = (0, model.image_dims[0]), x_range = (0, model.image_dims[1]))
        warp_image_plot.image_rgba(image = 'image', x=0, y=0, dw=model.image_dims[0], dh=model.image_dims[1], source=warp_source)
        ...
        image_plot = figure(title = 'Predicted Texture', y_range = (0, model.image_dims[0]), x_range = (0, model.image_dims[1]) )
        image_plot.image_rgba( image = 'image', x=0, y=0, dw=model.image_dims[0], dh=model.image_dims[1], source=texture_source)
        ###########################################################################
